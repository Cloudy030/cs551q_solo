...
class TestModel(TestCase):
  ...
  def test_year(self):
    ...
    self.assertEqual(year.id, 639)
    self.assertEqual(year.year_no, "2010")

  def test_genre(self):
    genre = Genre.objects.get(pk=190)
    self.assertEqual(genre.id, 190)
    self.assertEqual(genre.genre_name, "Simulation")

  def test_platform(self):
    platform = Platform.objects.get(pk=393)
    self.assertEqual(platform.id, 393)
    self.assertEqual(platform.platform_name, "SAT")

  def test_publisher(self):
    publisher = Publisher.objects.get(pk=2073)
    self.assertEqual(publisher.id, 2073)
    self.assertEqual(publisher.publisher_name, "Eidos Interactive")
